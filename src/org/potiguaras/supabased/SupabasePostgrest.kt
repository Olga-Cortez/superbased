...
@DesignerComponent(
    version = 59,
    versionName = "1.0",
    description = "Extension block for using Postgrest related stuff",
    iconName = "icon. png"
)
@Suppress("FunctionName")
class SupabasePostgrest(container: ComponentContainer) :
    AndroidNonvisibleComponent(container. `$form`()), OnDestroyListener, OnStopListener {
    ...
    @SimpleProperty(description = "Current table name to be used for database operations")
    ...
    @SimpleProperty(description = "Current columns list to be used for database operations ")
    ...
    fun SelectFrom(
        ...
        scope.launch {
            try {
                ...
                handleError(e, "SELECT", tableName)
            }
        }
    }
    ...
    fun InsertInto(
        ...
        scope.launch {
            try {
                ...
                handleError(e, "INSERT", tableName)
            }
        }
    }
    ...
    fun UpdateTable(
        ...
        scope.launch {
            try {
                ...
                handleError(e, "UPDATE", tableName)
            }
        }
    }
    ...
    fun DeleteFrom(
        ...
        scope.launch {
            try {
                ...
                handleError(e, "DELETE", tableName)
            }
        }
    }
    ...
    fun CallRPC(
        ...
        scope.launch {
            try {
                ...
                handleError(e, "RPC", functionName)
            }
        }
    }
        ...
        when (operator) {
            ...
            FilterOperator.IN -> {
                if (value is YailList) {
                    ...
                    if (list.isNotEmpty() && ! negate) {
                        // For IN operator, wrap all eq conditions in an OR block
                        filterBuilder.or {
                            for (item in list) {
                                eq(column, item)
                            }
